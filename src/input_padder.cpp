...
void InputPadder::setPadding(std::span<const int> padding) {
    if (padding.size() != 4)
    {
        throw std::invalid_argument("Padding must have exactly 4 values");
    }
    std::copy(padding.begin(), padding.end(), pad_.begin());
}